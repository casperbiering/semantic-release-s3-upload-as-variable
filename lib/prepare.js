...
async function handleDestination({ pluginConfig, context, destination }) {
  ...
  const putObjectParams = {
    ...
    Body: fs.createReadStream(pluginConfig.sourceFile),
  };
...
module.exports = async (pluginConfig, context) => {
  ...
  // TODO: remove
  // context.nextRelease = { version: "v123.123.123", notes: "" };
  // context.branch = { name: "main" };
  // console.log('prepare', pluginConfig, context);
  ...
  _.each(pluginConfig.fileGlobs, (pattern) => {
    glob(
      pattern,
      {
        cache: globCache,
